...
fsize=$(ls -l $WORKSPACE/$IMAGE_FILE | awk '{print $5}')
if [ "$fsize" -lt "$((20 * 1024 * 1024 * 1024))" ]; then
	qemu-img create -f qcow2 -o preallocation=metadata $WORKSPACE/newdisk.qcow2 20G || exit 1
	virt-resize --expand /dev/sda1 $WORKSPACE/$IMAGE_FILE $WORKSPACE/newdisk.qcow2 || exit 1
	mv -f $WORKSPACE/newdisk.qcow2 $WORKSPACE/$IMAGE_FILE || exit 1
else
